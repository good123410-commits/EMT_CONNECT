...
type TrainingCategoryManageModalProps = {
  open: boolean;
  onClose: () => void;
  onChanged: () => void;
};
...
export function TrainingCategoryManageModal({ open, onClose, onChanged }: TrainingCategoryManageModalProps) {
  const titleId = useId();
  ...
  const reload = async () => {
    setLoading(true);
    try {
      setCategories(await adminListTrainingCategories());
    } catch (err) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await adminUpsertTrainingCategory({
        name: form.name,
        slug,
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      await adminDeleteTrainingCategory(id);
  ...
  return (
    <div className="modal-overlay" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <p className="modal-desc">카테고리를 추가하거나 삭제하면 교육 등록 폼과 사용자 화면에 즉시 반영됩니다.</p>
