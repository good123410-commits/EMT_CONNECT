      ...
      <SafeAreaView className="flex-1">
        <ScrollView contentContainerClassName="px-6 pb-10 pt-6">
          <View className="mb-6 items-center">
            ...
            <Text className="text-center text-xl font-bold text-kemix-text">{COMMUNITY_PLEDGE_TITLE}</Text>
          ...
          <View className="rounded-2xl border border-green-200 bg-kemix-surface p-4">
            {COMMUNITY_PLEDGE_ITEMS.map((item, index) => (
              <View key={item} className={`flex-row ${index > 0 ? 'mt-4' : ''}`}>
                ...
                <Text className="flex-1 text-sm leading-6 text-kemix-text">{item}</Text>
              </View>
            ))}
          </View>

          <Pressable
            className="mt-6 flex-row items-start rounded-2xl border border-kemix-border bg-kemix-surface p-4"
            onPress={() => setChecked((v) => !v)}
          >
            <View
              className={`mr-3 mt-0.5 h-5 w-5 items-center justify-center rounded border ${
                checked ? 'border-green-600 bg-green-600' : 'border-kemix-border bg-kemix-surface'
              }`}
            >
              {checked ? <Ionicons name="checkmark" size={14} color="#fff" /> : null}
            </View>
            <Text className="flex-1 text-sm leading-6 text-kemix-text">
