  ...
  return (
    <View className="rounded-2xl border border-kemix-border bg-kemix-surface p-4">
      <View className="mb-3 flex-row items-center">
        ...
        <Text className="ml-2 text-sm font-semibold text-kemix-text">누출원 (사고 지점)</Text>
      </View>

      <View className="relative ml-2 border-l-2 border-dashed border-kemix-border pl-6">
        {zones.map((zone) => {
          const widthPercent = Math.min(100, (zone.distanceM / max) * 100);
          return (
            <View key={zone.label} className="mb-4">
              <View className="mb-1 flex-row items-center justify-between">
                <Text className="text-sm font-semibold text-kemix-text">{zone.label}</Text>
                <Text className="text-sm font-bold" style={{ color: zone.color }}>
                  {zone.distanceM}m
                </Text>
              </View>
              <View className="h-3 overflow-hidden rounded-full bg-kemix-elevated">
                <View
                  className="h-full rounded-full"
                  style={{ width: `${widthPercent}%`, backgroundColor: zone.color }}
                />
              </View>
              <Text className="mt-1 text-xs text-kemix-text-secondary">{zone.description}</Text>
            </View>
          );
        })}
      </View>

      <View className="mt-2 flex-row items-center rounded-lg bg-kemix-bg px-3 py-2">
        <Ionicons name="information-circle-outline" size={16} color="#64748b" />
        <Text className="ml-2 flex-1 text-xs text-kemix-text-secondary">
