      ...
      <SafeAreaView className="flex-1 bg-kemix-bg">
        <View className="flex-row items-center justify-between border-b border-kemix-border bg-kemix-surface px-4 py-3">
          <View className="flex-1 pr-3">
            <Text className="text-lg font-bold text-kemix-text">{SERVICE_POLICY_TITLE}</Text>
            {requireAcknowledgment ? (
              <Text className="mt-0.5 text-xs text-kemix-text-secondary">커뮤니티 이용 전 필수 확인</Text>
            ) : null}
          </View>
          {!requireAcknowledgment ? (
        ...
        {requireAcknowledgment ? (
          <View className="border-t border-kemix-border bg-kemix-surface px-4 pb-6 pt-4">
            <Pressable
          ...
          <View className="border-t border-kemix-border bg-kemix-surface px-4 pb-6 pt-4">
            <Pressable
              className="items-center rounded-2xl border border-kemix-border py-3.5 active:bg-kemix-bg"
              onPress={() => onClose?.()}
            >
              <Text className="font-semibold text-kemix-text">닫기</Text>
            </Pressable>
          </View>
        )}
      </SafeAreaView>
